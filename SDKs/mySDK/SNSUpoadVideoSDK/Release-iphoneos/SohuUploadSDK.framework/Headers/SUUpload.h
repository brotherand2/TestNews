...
#import "SUUploadReport.h"

@interface SUUpload : NSObject

@property(nonatomic,assign)BOOL isTestEnviroment;
...
//passport用户名字
//gid 设备的唯一ID
-(void)report:(NSArray<SUUploadReport*>*) info uploadFrom:(NSString *)from token:(NSString*)token passport:(NSString *) passport gid:(NSString*)gid appid:(NSString*)appid ua:(NSString*)ua andResult:(void(^)(NSDictionary *result))repostResult;
