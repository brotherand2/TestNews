...
#define SNNewsLogin_HalfPhoneView_CountDownTime 60 //登录页

@interface SNHalfPhoneView () <SNNewsLoginTextFieldDelegate>

@end

@implementation SNHalfPhoneView
...
- (void)countDownTime {
    __block int timeout = SNNewsLogin_HalfPhoneView_CountDownTime; //倒计时时间
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if (timeout<=0) { //倒计时结束，关闭
            ...
            int seconds = timeout % (SNNewsLogin_HalfPhoneView_CountDownTime+1);
...
- (void)hiddenPassword:(BOOL)show{
    if (show) {
        self.vcodeField.hidden = YES;
        _sendVerifyCodeBtn.hidden = YES;
        _verifyCodeLabel.hidden = YES;
    }
    else{
        self.vcodeField.hidden = NO;
        _sendVerifyCodeBtn.hidden = NO;
...
#pragma mark - SNNewsLoginTextFieldDelegate

-(void)textFieldDidChangeText:(SNNewsLoginTextField *)textField{
    if (textField == _phoneField) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(showHalfThirdView:)]) {
            if ([textField text].length>0) {
                [self.delegate showHalfThirdView:YES];
                [self resetVerifyCodeLabelStatus];//清空手机号 重置验证码
            }
            else{
                ...
                [self.delegate showHalfThirdView:NO];
            }
        }
    }
    else if (textField == _pcodeField){
        if (self.pcodeField.hidden == NO) {//如果输入的是图形验证码
        ...
        if ([_vcodeField text].length>=6) {
            //[_vcodeField resignFirstResponder];
            if (self.delegate && [self.delegate respondsToSelector:@selector(loginClick:)]) {
                [self.delegate loginClick:nil];
            }
        }
    }
}
