...
+ (SNTimelineTrendItem *)timelineTrendFromDic:(NSDictionary *)itemDic
{
    ...
    if (itemDic)
    {
        item = [[SNTimelineTrendItem alloc] init];
//        item.trendType = type;
        //默认不展开用户发表内容
        item.isOpenUgc = NO;
        item.needOpenUgc = NO;
        ...
        item.actType    = [itemDic intValueForKey:@"actType" defaultValue:501];
        
        //在数据解析层对不符合客户端动态模版的数据结构适配
        if (item.trendType == kSNTimelineItemTypeUGC && item.actType == SNTimelineActTypeUGCPublic) {
            item.trendType = kSNTimelineItemTypeArticle;
        }
...
+ (void)actorInfoFromDic:(NSDictionary *)infoDic trendItem:(SNTimelineTrendItem *)obj {
    ...
    if (actorDic) {
        obj.userNickName = [actorDic stringValueForKey:kTrendObjectKeyNickName defaultValue:@"搜狐网友"];
...
//        NSString *emoticonContent = [timelineItem.content replaceSubStringWithSpace:commentEmoticonPattern];
        ...
        while (index < timelineObj.commentsArray.count && index < kTimelineMaxCommentDisplayNum) {
            ...
            CGFloat maxHeight = [attributedStr getHeightWithWidth:kTLCommentsViewTextWidth
                                                     maxLineCount:SNTIMELINE_SHAREINFO_COMMENT_MAX_WIDTHOUT_MORE
                                                             font:[UIFont systemFontOfSize:kTLCommentsViewTextFontSize]];
...
+ (CGFloat)heightForTimelineOriginalContent:(SNTimelineTrendItem *)trendItem {
    ...
    switch (trendItem.trendType) {
        ...
        case kSNTimelineItemTypeUGC:
            height = [SNTimelineTrendItem heightForUGCOriginal:trendItem.originContentObj actType:trendItem.actType];
            break;
    }
    return height;
}
...
+ (CGFloat)heightForArticleOriginal:(SNTimelineOriginContentObject *)originObj {
    ...
    //原文来源高度
    if (originObj.fromString.length > 0) {
    ...
    else if (originObj.abstract.length > 0) {
        CGFloat maxHeight = [SNLabel heightForContent:originObj.abstract
                                             maxWidth:width
                                                 font:kTLOriginalContentAbstractFontSize
...
//ugc动态模版frame
+ (CGFloat)heightForUGCOriginal:(SNTimelineOriginContentObject *)originObj actType:(SNTimelineActType)type{
    CGFloat height = .0f;
    if(type != SNTimelineActTypeUGCPublic) {
        CGFloat width = kTLOriginalContentWidth - 2 * kTLOriginalContentTextSideMargin;
...
+ (void)SNTimelineTrendSendCmtSuc:(NSArray *)trendItems info:(NSDictionary *)cmtInfo
{
    ...
    for (SNTimelineTrendItem *item in trendItems) {
        if (actId.length > 0 && [item.actId isEqualToString:actId]) {
            SNTimelineCommentsObject *cmt = [SNTimelineCommentsObject new];
            cmt.actId = actId;
...
+ (SNTimelineOriginContentObject *)timelineOriginContentObjFromDic:(NSDictionary *)originInfoDic
{
    ...
    if (originInfoDic && [originInfoDic isKindOfClass:[NSDictionary class]]) {
        aObj = [SNTimelineOriginContentObject new];
...
//        aObj.sourceType = [originInfoDic intValueForKey:@"sourceType" defaultValue:0];
        aObj.sourceType = [[SNUtility getShareNewsSourceType:[originInfoDic stringValueForKey:@"url" defaultValue:@""] type:0] intValue];
        ...
        aObj.description = aObj.abstract;
        aObj.ugcWordLimit = [originInfoDic intValueForKey:@"ugcWordLimit" defaultValue:0];
...
//        NSString *picsString = [originInfoDic stringValueForKey:@"pics" defaultValue:nil];
        NSString *picsString = [originInfoDic stringValueForKey:@"imageUrl" defaultValue:nil];
        if (picsString.length > 0) {
//            NSArray *imagesUrl = [picsString componentsSeparatedByString:@","];
            NSArray *imagesUrl = [NSArray arrayWithObject:picsString];
...
//        aObj.link = [originInfoDic stringValueForKey:@"link" defaultValue:nil];
        aObj.link = [originInfoDic stringValueForKey:@"url" defaultValue:nil];
        if (aObj.link.length == 0) {
            aObj.link = [originInfoDic stringValueForKey:@"webUrl" defaultValue:nil];
        }
...
+ (SNTimelineOriginContentObject *)timelineOriginContentObjFromDic:(NSDictionary *)originInfoDic
    ...
    switch (obj.trendType) {
        ...
        case kSNTimelineItemTypeLive:
        case kSNTimelineItemTypeSub:
            return [SNTimelineOriginContentObject timelineArticleOriginalObjFromDic:originInfoDic trendItem:obj];
        case kSNTimelineItemTypePeople:
            return [SNTimelineOriginContentObject timelinePeopleOriginalObjFromDic:originInfoDic];
        case kSNTimelineItemTypeUGC:
            return [SNTimelineOriginContentObject timelineUGCOriginalObjFromDic:originInfoDic];
    }
    return nil;
}
...
//            NSArray *imagesUrl = [picsString componentsSeparatedByString:@","];
        ...
        //服务器字段通过actype来区分UGC形态，导致还是绑定与业务逻辑，而不是UI形式
        if(obj.actType == SNTimelineActTypeUGCPublic) {
            obj.ugcAudUrl = [originInfoDic stringValueForKey:@"audUrl" defaultValue:nil];
            obj.ugcAudLen = [originInfoDic intValueForKey:@"audLen" defaultValue:0];
            aObj.description = [originInfoDic stringValueForKey:@"shareContent" defaultValue:nil];
        }
    }
...
+ (SNTimelineOriginContentObject *)timelinePeopleOriginalObjFromDic:(NSDictionary *)originInfo
{
    ...
    if (originInfoDic && [originInfoDic isKindOfClass:[NSArray class]]) {
        ...
        for (NSDictionary *peopleInforDic in originInfoDic) {
            SNTimelineTrendTopObject *aTopObj = [SNTimelineTrendTopObject new];
...
//        }
//        else {
            aObj.ugcAudUrl = [originInfoDic stringValueForKey:@"audUrl" defaultValue:nil];
            aObj.ugcAudLen = [originInfoDic intValueForKey:@"audLen" defaultValue:0];
            aObj.sourceType = SNShareSourceTypeUGC;
//        }
    }
...
+ (SNTimelineOriginContentObject *)timelineOriginContentObjFromXMLObj:(TBXMLElement *)xmlElm {
    ...
    if (xmlElm) {
        obj = [SNTimelineOriginContentObject new];
        ...
        //解析二代协议不掉stringByRemovingHTMLTags 会在&符号截断
        obj.link = [TBXML textForElement:[TBXML childElementNamed:@"link" parentElement:xmlElm]];
        obj.fromLink = [TBXML textForElement:[TBXML childElementNamed:@"fromLink" parentElement:xmlElm]];
        ...
        obj.ugcWordLimit = [[TBXML textForElement:[TBXML childElementNamed:@"ugcWordLimit" parentElement:xmlElm]] intValue];
...
//            NSArray *imageUrls = [picUrl componentsSeparatedByString:@","];
            NSArray *imageUrls = [NSArray arrayWithObject:picUrl];
...
{
    SNTimelineOriginContentObject *aObj = nil;
    if (originInfo && [originInfo isKindOfClass:[NSDictionary class]]) {
        aObj = [SNTimelineOriginContentObject new];
        aObj.contentId   = [originInfo stringValueForKey:@"contentId" defaultValue:nil];
        aObj.sourceType = [originInfo intValueForKey:@"sourceType" defaultValue:0];
        aObj.abstract = [originInfo stringValueForKey:@"abstract" defaultValue:nil];
        aObj.picUrl = [originInfo stringValueForKey:@"picUrl" defaultValue:nil];
        aObj.link   = [originInfo stringValueForKey:@"link" defaultValue:nil];
    }
    return aObj;
}
...
- (NSDictionary *)toDictionary {
    ...
    [dic setObject:@(self.ugcWordLimit) forKey:@"ugcWordLimit"];
...
void CommentRunDelegateDeallocCallback(void* refCon ){
    
}

CGFloat CommentRunDelegateGetAscentCallback( void *refCon ){
...
CGFloat CommentRunDelegateGetDescentCallback(void *refCon){
    UIFont *font = (__bridge UIFont *)refCon;
    CTFontRef fontRef = CTFontCreateWithName((CFStringRef)font.fontName, font.pointSize, NULL);
...
CGFloat CommentEmoticonRunDelegateGetWidthCallback(void *refCon){
    UIFont *font = (__bridge UIFont *)refCon;
...
+ (SNTimelineCommentsObject *)timelineCommentObjFromDic:(NSDictionary *)commentInfoDic {
    ...
    if (commentInfoDic && [commentInfoDic isKindOfClass:[NSDictionary class]]) {
        aObj = [SNTimelineCommentsObject new];
        ...
        if (userDic) {
            ...
            aObj.nickName = [userDic stringValueForKey:@"nickName" defaultValue:@"搜狐网友"];
...
- (NSMutableAttributedString *)setCommentAttributedStr
{
    ...
    attContent = [[SNEmoticonManager sharedManager] parseEmoticonFromText:attContent emoticon:imageRangeDic];
    attributedStr = [[NSMutableAttributedString alloc] initWithString:attContent];
    ...
    NSRange textRagne = NSMakeRange(userStr.length, attributedStr.length - userStr.length);
    ...
    if (textRagne.length + textRagne.location <= attributedStr.length) {
        [attributedStr setTextColor:textColor range:textRagne];
    }
    if (replayedUserRange.length > 0 && replayedUserRange.length <  attributedStr.length) {
    ...
    if (imageRangeDic.count > 0) {
        ...
        callbacks.dealloc = CommentRunDelegateDeallocCallback;
        callbacks.getAscent = CommentRunDelegateGetAscentCallback;
        callbacks.getDescent = CommentRunDelegateGetDescentCallback;
        callbacks.getWidth = CommentEmoticonRunDelegateGetWidthCallback;
        CTRunDelegateRef runDelegate = CTRunDelegateCreate(&callbacks, (__bridge void * _Nullable)([UIFont systemFontOfSize:kTLCommentsViewTextFontSize]));
        ...
        if (imageRangeDic) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (rangeKey.location + rangeKey.length <= attributedStr.length) {
                    [attributedStr addAttribute:@"emoticonType" value:(id)@(emoticon.type) range:rangeKey];
                    //区别重复表情
                    [attributedStr addAttribute:@"emoticonIndex" value:(id)@(i) range:rangeKey];
                    [attributedStr addAttribute:@"emoticonImage" value:(id)emoticon.emoticonImage range:rangeKey];
                    [attributedStr addAttribute:(NSString *)kCTRunDelegateAttributeName value:(__bridge id)runDelegate range:rangeKey];
                }
            }
        }
    }
...
+ (SNTimelineTrendTopObject *)timelineTopObjFromDic:(NSDictionary *)topDic {
    ...
    if (topDic && [topDic isKindOfClass:[NSDictionary class]]) {
        aObj = [SNTimelineTrendTopObject new];
        NSString *timeString = [NSString stringWithFormat:@"%lf", [[NSDate date] timeIntervalSince1970]*1000];
