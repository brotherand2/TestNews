...
// UI
#import "SNScrollViewDelegate.h"
#import "SNScrollViewDataSource.h"
...
//    _pages = [[NSMutableArray alloc] initWithCapacity:_maxPages];
    _pages = [[NSMutableArray alloc] initWithCapacity:50];
	...
	self.backgroundColor = RGBACOLOR(0, 0, 0, 0.1);//make it clickable when view is transparent, 
												   //if the page does not fully fill screen, the margin area is not clickable
...
- (BOOL)draggingFromLeftEdge {
    return _pageEdges.left > 0 && [self isFirstPage];
}

- (BOOL)draggingFromRightEdge {
    return _pageEdges.left < 0 && [self isLastPage];
}
...
//  }
    ...
    if (pageIndex > _pages.count - 1) {
        // 超出当前数量后，对_pages扩容一次，10是为了避免每次都扩容，一次多扩点儿
        for (NSInteger i=_pages.count; i<pageIndex+10; ++i) {
            [_pages addObject:[NSNull null]];
        }
    }
    ...
    if ((NSNull*)page == [NSNull null]) {
        if (create) {
            ...
            SNDebugLog(@"replaceObjectAtIndex %d", pageIndex);
            if (page) {
...
//  else {
//	[_dataSource scrollView:self pageAtIndex:pageIndex];
//  }
...
-(void)inertiaAnimator {
  ...
  if (abs(_inertiaSpeed.x) < 0.05 && abs(_inertiaSpeed.y) < 0.05) {
    // Stop animator.
      // 放大状态下 结束timer时不需要再对index进行动画调整 否则index会重置动画会乱跳 by jojo
//      [self stopAnimation:NO];
      {
          if (_animationTimer) {
              [_animationTimer invalidate];
...
- (UIEdgeInsets)squareEdgesUsingZoomScale:(CGFloat)newScale andAnchorPoint:(CGPoint)anchorPoint {
  ...
  SNDebugLog( @"page-with: %f", self.pageWidth );
  SNDebugLog( @"resized-width: %f", newStretched.width);
  SNDebugLog( @"old-width: %f", self.stretchedWidth);
  SNDebugLog( @"actual left: %f", _pageEdges.left );
  SNDebugLog( @"actual right: %f", _pageEdges.right );
  SNDebugLog( @"touch: %f", anchorPoint.x);
  SNDebugLog( @"point: %f", anchorPoint.x);
  SNDebugLog( @"B: %f", scaledRanges.x);
  SNDebugLog( @"C: %f", normalRanges.x);
  SNDebugLog( @"left: %f", edges.left);
  SNDebugLog( @"right: %f", edges.right);
  SNDebugLog( @"/////////////////////////////////////////////////////////////////////////////////");
        ...
        // Double tap, zoom out to fit or zoom in to the 1/3 of the maximum scale.
          ...
          if (self.zoomed) {
            ...
            [self setZoomScale:_maximumZoomScale / 1.3  withPoint:pointToZoom animated:YES];
              _inertiaSpeed = CGPointZero; // 解决双击跳跃的问题 by jojo
          }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setDataSource:(id<SNScrollViewDataSource>)dataSource {
