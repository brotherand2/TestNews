...
@property (nonatomic, weak)id pushReceivedNotificationObserver;
@property (nonatomic, weak)id downloaderWillShowNotificationObserver;
...
//    _scrollView.alpha = themeImageAlphaValue();
    ...
    [_slideshows.photos enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        else if ([obj isKindOfClass:[NSDictionary class]])
        {
            ...
            slideshowView.adDataCarrier = obj[@"adData"];
            [_scrollView addSubview:slideshowView];
            [_slideShowViews addObject:slideshowView];
        }
    }];
    ...
    if (subObj)
    {
        ...
        _headerView.newsId = self.slideshows.newsId ? : @"0";
    ...
    [_footerView setBackButtonActionBlock:^{
        ...
        //发表评论操作
        if (blockSelf.delegate && [blockSelf.delegate respondsToSelector:@selector(photoViewComment)])
        {
            [blockSelf.delegate performSelector:@selector(photoViewComment)];
        }
    } downloadButtonActionBlock:^{
        //下载图片操作
        UIImage *saveImage = [[blockSelf slideshowViewAtIndex:blockSelf.slideshowIndex] image];
        ...
        //分享操作
        if (blockSelf.delegate && [blockSelf.delegate respondsToSelector:@selector(photoViewWillShare:)])
        {
            [blockSelf.delegate photoViewWillShare:blockSelf.slideshowIndex];
        }
    ...
    UISwipeGestureRecognizer *recognizerUp = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeGestureUpOrDown:)];
    [recognizerUp setDirection:UISwipeGestureRecognizerDirectionUp];
    [self.view addGestureRecognizer:recognizerUp];
    //2017-02-08 wangchuanwen update begin 增加下滑退出大图模式
    UISwipeGestureRecognizer *recognizerDown = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeGestureUpOrDown:)];
    [recognizerDown setDirection:UISwipeGestureRecognizerDirectionDown];
    [self.view addGestureRecognizer:recognizerDown];
    
    [SNNotificationManager addObserver:self selector:@selector(closeSNPicturesSlideshowViewController) name:GallerySliderPicturesNotification object:nil];
    //2017-02-08 wangchuanwen update end
    
    self.pushReceivedNotificationObserver = [SNNotificationManager addObserverForName:kNotifyDidReceive object:nil queue:nil usingBlock:^(NSNotification *note) {
        [SNNotificationManager removeObserver:blockSelf.pushReceivedNotificationObserver], blockSelf.pushReceivedNotificationObserver = nil;
        //关闭此页面
        [blockSelf closeSNPicturesSlideshowViewController];
    }];
    self.downloaderWillShowNotificationObserver = [SNNotificationManager addObserverForName:kNotificationWillOpenDownloader object:nil queue:nil usingBlock:^(NSNotification *note) {
        [SNNotificationManager removeObserver:blockSelf.downloaderWillShowNotificationObserver], blockSelf.downloaderWillShowNotificationObserver = nil;
        //关闭此页面
        [blockSelf closeSNPicturesSlideshowViewController];
    }];
...
- (void)removeObserver
{
    [SNNotificationManager removeObserver:self.pushReceivedNotificationObserver], self.pushReceivedNotificationObserver = nil;
    [SNNotificationManager removeObserver:self.downloaderWillShowNotificationObserver], self.downloaderWillShowNotificationObserver = nil;
...
- (SNSlideshowView *)slideshowViewAtIndex:(NSInteger)index
{
...
{
    if (_slideshowIndex >= 0 && _slideshowIndex < _slideshows.photos.count) {
        id obj = _slideshows.photos[_slideshowIndex];
        
        if ([obj isKindOfClass:[NSDictionary class]])
        {
            NSDictionary *adDic = (NSDictionary *)obj;
            SNAdDataCarrier *adData = adDic[@"adData"];
            
            if (nil != adData
                && [adData isKindOfClass:[SNAdDataCarrier class]]
                && nil != adData.adInfoDic)
            {
                NSString *clickUrl = adData.adInfoDic[@"click_url"];
                
                if (nil != clickUrl) {
                    [adData reportForClickTrack];
                    [SNUtility openProtocolUrl:clickUrl];
...
- (void)handleSwipeGestureUpOrDown:(UISwipeGestureRecognizer *)gestureRecognizer {
