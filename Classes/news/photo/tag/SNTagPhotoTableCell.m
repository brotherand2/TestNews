...
+ (CGFloat)tableView:(UITableView*)tableView rowHeightForObject:(id)object {
    ...
    if (dataItem.row == 0) {
        ...
        NSInteger rows = [allCategories count]%4 == 0 ? [allCategories count]/4 : [allCategories count]/4 + 1;
    ...
    if (categories) {
        ...
        UIColor *selectedColor =  [UIColor colorFromString:[[SNThemeManager sharedThemeManager] currentThemeValueForKey:kGroupPhotoTagSelectedTextColor]];
         for (int i = 0; i < [categories count]; i++) {
             ...
             NSString *normalIconFileName = [self categoryBgByName:category.icon];
             [cBtn setImage:[UIImage imageNamed:normalIconFileName] forState:UIControlStateNormal];
             NSString *selectedIconFileName = [self categoryHlBgByName:category.icon];
             [cBtn setImage:[UIImage imageNamed:selectedIconFileName] forState:UIControlStateHighlighted];
             [cBtn setImage:[UIImage imageNamed:selectedIconFileName] forState:UIControlStateSelected];
...
- (void)setObject:(id)object {
     if (object != self.item) {
        ...
        UIImage * bgImage = [UIImage imageNamed:@"category_bg.png"];
        if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"5.0")) {
            bgImage = [bgImage resizableImageWithCapInsets:UIEdgeInsetsMake(10, 10, 10, 10)];
        } else {
            bgImage = [bgImage stretchableImageWithLeftCapWidth:10 topCapHeight:10];
        ...
        titleLabel.textColor = [UIColor colorFromString:[[SNThemeManager sharedThemeManager] currentThemeValueForKey:kGroupPhtotCellTitleColor]];
        ...
        lineView = [[UIImageView alloc] init];
         ...
         lineView.image = [UIImage imageNamed:@"sep_line.png"];
