...
//  DXPopover.m
//
//  Created by qz on 2017.
//  Copyright (c) 2017 sn. All rights reserved.
//
...
@implementation SNCellPopover

+ (instancetype)popover
{
    return [[SNCellPopover alloc] init];
}
...
- (instancetype)init
{
    ...
    if (self) {
        self.arrowSize = CGSizeMake(11.0, 7.0);
        ...
        self.betweenAtViewAndArrowHeight = -2;
    }
    return self;
}
...
- (void)showAtView:(UIView *)atView popoverPostion:(DXPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView
{
    ...
    CGRect atViewFrame = CGRectZero;
    if (atView.superview) {
        atViewFrame = [atView.superview convertRect:atView.frame toView:containerView];
    }

    BOOL upCanContain = CGRectGetMinY(atViewFrame) >= contentViewHeight+betweenArrowAndAtView;
    BOOL downCanContain = (CGRectGetHeight(containerView.bounds) - 44 - (CGRectGetMaxY(atViewFrame)+betweenArrowAndAtView)) >= contentViewHeight;
    
    CGPoint atPoint = CGPointMake(CGRectGetMidX(atViewFrame)-2, 0);
...
- (void)show
{
    ...
    if (self.animationSpring) {
        [UIView animateWithDuration:self.animationIn delay:0 usingSpringWithDamping:0.84 initialSpringVelocity:3 options:UIViewAnimationOptionCurveEaseInOut animations:^{
...
- (void)drawRect:(CGRect)rect
{
    ...
    UIColor *contentColor = [[SNThemeManager sharedThemeManager] currentThemeColorForKey:kThemeBg4Color];
