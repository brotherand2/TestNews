...
@implementation SNSpecialArticleNewsContentWorker
...
- (void)startInThread {
    ...
    for (SNNewsContentWorkerNews *_news in _newsArray) {
        ...
        NSString *termID	    = _news.termID;
        SNArticle *article	= [SNArticle newsForDownloadWithNewsId:newsId termId:termID paramsDic:nil];
        if (!!article) {
            ...
            SNDebugLog(@"===INFO: Main thread:%d, special news article content existed in local, so neednt resave.", [NSThread isMainThread]);
        }
    }
