...
-(void)genieInTransitionWithDuration:(NSTimeInterval)duration destinationRect:(CGRect)destRect destinationEdge:(BCRectEdge)destEdge completion:(void (^)())completion superview:(UIView*)aSuperview{
...
-(void)genieOutTransitionWithDuration:(NSTimeInterval)duration startRect:(CGRect)startRect startEdge:(BCRectEdge)startEdge completion:(void (^)())completion superview:(UIView*)aSuperview;{
    [self genieTransitionWithDuration:duration
                                 edge:startEdge
...
-(void)printBCBezierCurves:(BCBezierCurve)first second:(BCBezierCurve)second
{
    SNDebugLog(@"first %f,%f", first.a.x, first.a.y);
    SNDebugLog(@"second %f,%f", second.a.x, second.a.y);
    SNDebugLog(@"\n");
}
                           ...
                           superview:(UIView*)aSuperview;
{
    ...
    BCSegment aPoints = bezierEndPointsForTransition(edge, [self convertRect:CGRectInset(self.bounds, xInset, yInset) toView:superview]);
    ...
    if (sign*(aPoints.a.v[axis] - bEndPoints.a.v[axis]) > 0.0f) {
        ...
        //SNDebugLog(@"Genie Effect ERROR: The distance between %@ edge of animated view and %@ edge of %@ rect is incorrect. Animation will not be performed!", edgeDescription(edge), edgeDescription(edge), reverse ? @"star" : @"destination");
        return;
    } else if (sign*(aPoints.a.v[axis] + sign*totalSize - bEndPoints.a.v[axis]) > 0.0f) {
        //SNDebugLog(@"Genie Effect Warning: The %@ edge of animated view overlaps %@ edge of %@ rect. Glitches may occur.",edgeDescription((edge + 2) % 4), edgeDescription(edge), reverse ? @"start" : @"destination");
    }
    
    UIView *containerView = [[UIView alloc] initWithFrame:[superview bounds]];
    containerView.clipsToBounds = superview.clipsToBounds; // if superview does it then we should probably do it as well
    ...
    for (int i = 0; i < totalIter; i++)
    {
        ...
        //SNDebugLog(@"progress=%f, t=%f, curveP=%f", progress, t, curveP);
        ...
        //[self printBCBezierCurves:first second:second];
