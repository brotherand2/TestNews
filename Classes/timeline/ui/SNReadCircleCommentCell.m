...
@property (nonatomic, retain) UIImage *topBgImage;
@property (nonatomic, retain) UIImage *middleBgImage;
@property (nonatomic, retain) UIImage *bottomBgImage;
...
+ (CGFloat)heightForReadCircleComment:(SNTimelineCommentsObject *)cmtObj objIndex:(int)index
{
    ...
    CGFloat maxHeight = [SNLabel heightForContent:cmtObj.content
    ...
    CGFloat maxHeightWithMore = [SNLabel heightForContent:cmtObj.content
                                                 maxWidth:kRCCommentTextWidth
                                                     font:kTLCommentsViewTextFontSize
                                               lineHeight:kTLCommentsViewTextLineHeight
                                             maxLineCount:SNTIMELINE_SHAREINFO_COMMENT_MAX_WIDTH_MORE];
    CGFloat textHeight = [SNLabel heightForContent:cmtObj.content
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        if (self.topBgImage && [self.topBgImage respondsToSelector:@selector(resizableImageWithCapInsets:)]) {
            self.topBgImage = [self.topBgImage resizableImageWithCapInsets:UIEdgeInsetsMake(_topImgHeight - 1, 1, 0, 5)];
        }
        else {
        ...
        if (self.middleBgImage && [self.middleBgImage respondsToSelector:@selector(resizableImageWithCapInsets:)]) {
            self.middleBgImage = [self.middleBgImage resizableImageWithCapInsets:UIEdgeInsetsMake(1, 0, 0, 1)];
        }
        else {
...
- (void)dealloc
{
    TT_RELEASE_SAFELY(_topBgImage);
    TT_RELEASE_SAFELY(_bottomBgImage);
    TT_RELEASE_SAFELY(_middleBgImage);
    TT_RELEASE_SAFELY(_cmtObj);
    TT_RELEASE_SAFELY(_headIconView);
    TT_RELEASE_SAFELY(_contentLabel);
    TT_RELEASE_SAFELY(_sepLineView);
...
- (void)setContent
{
    ...
    _contentLabel.text = self.cmtObj.content;
    ...
    if(self.cmtObj.isFolder && self.cmtObj.needFolder) {
        ...
        [_moreButton setActionBlock:^(UIControl *control) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(snTrendCmtBtnOpenMore:)]) {
                [self.delegate snTrendCmtBtnOpenMore:self.cmtObj.commentId];
            }
