...
#define kTopicPostCommentToTimeline     (@"发表评论")
...
#define kTrendApprovalParam (@"userActV2?action=topAct&actId=%@&isTop=%d&spid=%@")
...
- (void)dealloc {
    TT_RELEASE_SAFELY(_dataId);
    TT_RELEASE_SAFELY(_allHeaderFields);
    [super dealloc];
}
...
//        userName = obj._uid;
    ...
    request.response = [[[SNTimelinePostResponse alloc] init] autorelease];
...
//    NSString *requestUrl = [NSMutableString stringWithFormat: @"http://10.10.76.14:8050/userAct?action=delUserAct&pid=%@&actId=%@", pid, actId];
    SNURLRequest *request = [SNURLRequest requestWithURL:requestUrl delegate:self];
...
{
    NSString *requestParam = [NSString stringWithFormat:kTrendApprovalParam, actId, type, spid];
    ...
    if (userInfo) {
        ...
        if ([userInfo.topic isEqualToString:kTopicPostCommentToTimeline] || [userInfo.topic isEqualToString:kTopicReplyCommentToTimeline]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (userInfo && infoDic) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:kTLTrendSendCommentSucNotification
                                                                        object:nil
                                                                      userInfo:infoDic];
                }
            });
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (userInfo && infoDic) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:kTLTrendCellDeleteNotification
                                                                        object:infoDic
                                                                      userInfo:nil];
                }
            });
...
- (void)request:(TTURLRequest *)request didFailLoadWithError:(NSError *)error {
    SNDebugLog(@"request fail load with error : %@ %@", error, [[[NSString alloc] initWithData:[error.userInfo objectForKey:@"responsedata"] encoding:NSUTF8StringEncoding] autorelease]);
    TTUserInfo *userInfo = request.userInfo;
    if ([userInfo.topic isEqualToString:kTopicPostCommentToTimeline] || [userInfo.topic isEqualToString:kTopicReplyCommentToTimeline]) {
