...
- (void)requestHotPhotos:(BOOL)isASyn {
	...
	if (!_request) {
		_request = [[SNURLRequest requestWithURL:url delegate:self isParamP:YES scookie:YES] retain];
...
- (void)parseJsonData:(id)aData {
    ...
    if ([newsData isKindOfClass:[NSArray class]]) {
        for (NSDictionary *newsDic in newsData) {
            ...
            TT_RELEASE_SAFELY(hotNews);
        }
        
        if ([(NSArray *)newsData count] > 0) {
            _page++;
            self.hasNoMore = NO;
        }
    }
...
-(void)dealloc {
    TT_RELEASE_SAFELY(timelineWhenViewReleased);
    TT_RELEASE_SAFELY(lastOffset);
    TT_RELEASE_SAFELY(offSet);
    TT_RELEASE_SAFELY(targetType);
    TT_RELEASE_SAFELY(typeId);
    TT_RELEASE_SAFELY(hotPhotos);
    TT_RELEASE_SAFELY(allPhotos);
