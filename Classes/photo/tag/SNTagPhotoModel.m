...
- (void)load:(TTURLRequestCachePolicy)cachePolicy more:(BOOL)more 
{
    ...
    if (!self.isLoading) {
        if (_isFirst) {
            ...
            TT_RELEASE_SAFELY(cachedCategories);
...
- (void)parseJsonData:(id)rootData {
    ...
    if ([categoriesData isKindOfClass:[NSArray class]]) {
        ...
        if (bNeedNotify) {
            [[NSNotificationCenter defaultCenter] postNotificationName:kPhotoChannelChangedNotification object:nil];
        }
    }
    ...
    if ([tagsData isKindOfClass:[NSArray class]]) {
        for (NSDictionary *tagDic in tagsData) {
            TagItem *tag = [[TagItem alloc] init];
...
-(void)dealloc {
    ...
    TT_RELEASE_SAFELY(allCategories);
    TT_RELEASE_SAFELY(oldCategories);
