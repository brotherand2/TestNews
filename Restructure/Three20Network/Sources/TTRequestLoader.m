...
- (void)dealloc {
  ...
  TT_RELEASE_SAFELY(_outputStream);
  [super dealloc];
}
...
- (BOOL)cancel:(TTURLRequest*)request {
  ...
  if (requestIndex != NSNotFound) {
    ...
    for (id<TTURLRequestDelegate> delegate in request.delegates) {
      ...
      // v5.2.0 加个保护
      if ([_requests count] > requestIndex) {
...
- (NSError*)processResponse:(NSHTTPURLResponse*)response data:(id)data {
  for (TTURLRequest* request in _requests) {
    ...
    if (!response
    ...
    } else if (response.statusCode == 416 || response.statusCode == 404) {
        NSString* filePath = [[TTURLCache sharedCache] cachePathForURL:request.urlPath];
      ...
      if (!error) {
        TTDCONDITIONLOG(TTDFLAG_URLREQUEST, @"  FAILED LOADING (%zd) %@", _response.statusCode, _urlPath);
        NSDictionary* userInfo = nil;
        if (data) {
            userInfo = [NSDictionary dictionaryWithObject:data forKey:kTTErrorResponseDataKey];
        }
  ...
  if (contentLength > _queue.maxContentLength && _queue.maxContentLength) {
    ...
    for (TTURLRequest* request in [[_requests copy] autorelease]) {
        ...
        if ([request isAllowResume]) {
            ...
            NSData *cachedData = [[TTURLCache sharedCache] dataForURL:self.urlPath];
            _resumeLength = [cachedData length];
        }
        request.totalContentLength += _resumeLength;
        request.totalBytesDownloaded += _resumeLength;
    }
    
    if (isResume) {
#ifndef DOWNLOAD_INTO_MEMORY
        NSString *downloadPath = [[TTURLCache sharedCache] cachePathForURL:self.urlPath];
        if (!_outputStream) {
            _outputStream = [[NSOutputStream alloc] initToFileAtPath:downloadPath append:YES];
        }
...
//        }
        ...
        NSData *cachedData = [[TTURLCache sharedCache] dataForURL:self.urlPath];
        if (cachedData && [cachedData length]) {
            [_responseData appendData:cachedData];
        }
#endif
    }
    else {
...
#ifndef DOWNLOAD_INTO_MEMORY
    if (!((_response.statusCode >= 200 && _response.statusCode < 300) || _response.statusCode == 304)) {
      return;
    }
...
#ifndef DOWNLOAD_INTO_MEMORY
    [_queue loader:self didLoadResponse:_response data:nil];
