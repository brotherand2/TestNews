...
- (void)removeAuthData
{
    ...
    NSArray* sinaweiboCookies = [cookies cookiesForURL:
                                [NSURL URLWithString:kSinaWeiboSDKAPIDomainUrl]];
...
- (void)request:(SinaWeiboRequest *)_request didFinishLoadingWithResult:(id)result
{
    if (_request == request)
    {
        SNDebugLog(@"access token result = %@", result);
...
- (BOOL)handleOpenURL:(NSURL *)url
{
    ...
    if ([urlString hasPrefix:self.ssoCallbackScheme])
    {
        ...
        else
        {
            ...
            // 用户取消了
            else if ([SinaWeiboRequest getParamValueFromUrl:urlString paramName:@"failed"]) {
                [self logInDidCancel];
            }
            else
            {
