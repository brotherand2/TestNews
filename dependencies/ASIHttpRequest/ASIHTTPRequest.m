...
#define kBandwidthTrackerCount                          (10)//ASI默认是5
...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		bandwidthUsageTracker = [[NSMutableArray alloc] initWithCapacity:kBandwidthTrackerCount];
...
//		while ((interval < 0 || [bandwidthUsageTracker count] > kBandwidthTrackerCount) && [bandwidthUsageTracker count] > 0) {
//			[bandwidthUsageTracker removeObjectAtIndex:0];
//			interval++;
//		}
//	}
    
    while (([bandwidthUsageTracker count] > kBandwidthTrackerCount) && [bandwidthUsageTracker count] > 0) {
        [bandwidthUsageTracker removeObjectAtIndex:0];
    }
    //SNDebugLog(@"bandwidthUsageTracker are %@", bandwidthUsageTracker);
...
+ (BOOL)isNetworkReachableViaWWAN
{
	return (  [[Reachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN
            ||[[Reachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableVia2G
            ||[[Reachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableVia3G
            ||[[Reachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableVia4G);
}
