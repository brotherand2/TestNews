...
@implementation COMPFMResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(COMPFMStatement *)statement usingParentDatabase:(COMPFMDatabase*)aDB {
    
    COMPFMResultSet *rs = [[COMPFMResultSet alloc] init];
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        NSLog(@"%s:%d COMPDatabase busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        NSLog(@"COMPDatabase busy");
        if (outErr) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"COMPFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(COMPFMDatabase *)newDb {
    _parentDB = newDb;
}
