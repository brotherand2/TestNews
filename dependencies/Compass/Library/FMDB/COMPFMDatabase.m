...
@interface COMPFMDatabase ()

- (COMPFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation COMPFMDatabase
...
// if ([COMPFMDatabase FMDBVersion] >= 0x0240) { … }
...
static int COMPFMDBDatabaseBusyHandler(void *f, int count) {
    COMPFMDatabase *self = (__bridge COMPFMDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &COMPFMDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to COMPFMDatabase changed,
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        COMPFMResultSet *rs = (COMPFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(COMPFMResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (COMPFMStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (COMPFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(COMPFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(COMPFMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    COMPFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The COMPFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The COMPFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The COMPFMDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The COMPFMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"COMPFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (COMPFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (COMPFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    COMPFMStatement *statement  = 0x00;
    COMPFMResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[COMPFMStatement alloc] init];
    ...
    rs = [COMPFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (COMPFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (COMPFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (COMPFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (COMPFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    COMPFMResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (COMPFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    COMPFMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[COMPFMStatement alloc] init];
...
int COMPFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int COMPFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? COMPFMDBExecuteBulkSQLCallback : nil, (__bridge void *) (block), &errmsg);
...
static NSString *COMPFMDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", COMPFMDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", COMPFMDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", COMPFMDBEscapeSavePointName(name)];
...
- (NSError*)inSavePoint:(void (^)(BOOL *rollback))block {
    ...
    NSString *errorMessage = NSLocalizedString(@"Save point functions require SQLite 3.7", nil);
    if (self.logsErrors) NSLog(@"%@", errorMessage);
    return [NSError errorWithDomain:@"COMPFMDatabase" code:0 userInfo:@{NSLocalizedDescriptionKey : errorMessage}];
#endif
}
...
void COMPFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void COMPFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &COMPFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &COMPFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation COMPFMStatement
