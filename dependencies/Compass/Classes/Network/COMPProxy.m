...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend
{
    ...
    [[COMPCompassManager sharedInstance] urlSessionTask:task totalBytesExpectedToSend:totalBytesExpectedToSend];
}
...
- (void)URLSession:(NSURLSession *)session task:(nonnull NSURLSessionTask *)task didFinishCollectingMetrics:(nonnull NSURLSessionTaskMetrics *)metrics {
    if (IS_IOS_VERSION_GREATER_THAN_OR_EQUAL_TO_10) {
        ...
        [[COMPCompassManager sharedInstance] urlSessionTask:task didFinishCollectingMetrics:metrics];
    }
}
 ...
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler
{
    ...
    [[COMPCompassManager sharedInstance] urlSessionTask:dataTask didReceiveResponse:response ts:[COMPHelper timestampInMiniseconds]];
}
