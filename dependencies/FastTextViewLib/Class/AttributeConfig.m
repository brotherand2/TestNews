...
- (void)setFont:(UIFont *)font {
    ...
    NSMutableDictionary *dictionary = [[self.attributes mutableCopy] autorelease];
    
    [dictionary setObject:(__bridge id)ctFont
...
- (void)setReadStyle:(NSInteger)readStyle {
    UIColor *color = nil; //数据库
    UIColor *strokeColor = nil; //数据库
    if (readStyle == 0) {
        //白天
    ...
    [dictionary setObject:(id)color.CGColor
                   forKey:(NSString *)kCTForegroundColorAttributeName];
    [dictionary setObject:(id)strokeColor.CGColor
                   forKey:(NSString *) kCTStrokeColorAttributeName];
    self.attributes = dictionary;
}

- (UIColor *)colorWithR:(NSInteger)r G:(NSInteger)g
                      B:(NSInteger)b alpha:(CGFloat)alpha {
    CGFloat red = [NSNumber numberWithInteger:r].floatValue/255;
    CGFloat green = [NSNumber numberWithInteger:g].floatValue/255;
    CGFloat blue = [NSNumber numberWithInteger:b].floatValue/255;
